...
function  church_admin_member_type()
{
    ...
    if(isset($_POST['current']))
    {
        ...
        echo'<div class="notice notice-success inline"><p>'.__('People reassigned','church-admin').'</p></div>';
    }


    echo'<p>'.__('Member Types can be sorted by drag and drop, for use in other parts of the plugin','church-admin').'</p>';
    echo'<table id="sortable" class="widefat striped"><thead><tr><th>'.__('Edit','church-admin').'</th><th>'.__('Delete','church-admin').'</th><th>'.__('Type','church-admin').'</th><th>'.__('Reassign','church-admin').'</th></tr></thead><tbody class="content">';
    ...
    foreach($member_type AS $id=>$membertype)
    {
        ...
        if(!$check)
        {
            $delete='<a onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;action=church_admin_delete_member_type&member_type_id='.$id,'delete_member_type').'">'.__('Delete','church-admin').'</a>';
            $reassign='';
        }
        else
        {
    ...
    echo'</tbody><tfoot><tr><th>'.__('Edit','church-admin').'</th><th>'.__('Delete','church-admin').'</th><th>'.__('Type','church-admin').'</th><th>'.__('Reassign','church-admin').'</th></tr></tfoot></table>';
    echo '
...
function church_admin_edit_member_type($member_type_id=NULL)
{
    ...
    if(isset($_POST['edit_member_type']))
    {
        ...
        echo'<div class="notice notice-success inline"><p>'.__('Member Type Updated','church-admin').'</p></div>';
...
function church_admin_delete_member_type($member_type_id=NULL)
{
    ...
    if($member_type_id)
    {
        ...
        echo'<div class="notice notice-success inline"><p><strong>'.__('Member Type Deleted','church-admin').'</strong></p></div>';
    }
    church_admin_member_type();
}
?>
