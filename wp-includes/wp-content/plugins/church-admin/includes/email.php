...
function church_admin_send_email($email_id=NULL)
{
	...
	$sql='SELECT a.first_name, a.email, a.people_id FROM '.implode(", ",array_filter($tables)).' WHERE a.household_id=b.household_id '.$other.' AND email_send!=0 AND gdpr_reason!="" GROUP BY a.email  ORDER BY a.last_name';
}
	...
	update_option('church_admin_from_email',stripslashes($_POST['from_email']));
		...
		if($results)
		{
			foreach($results AS $row)
			{
				...
				$unsubLink='<a href="'.site_url().'?ca_unsub='.md5($row->people_id).'">'.__('Unsubscribe','church-admin').'</a>';
				...
				if(!empty($schedule)||get_option('church_admin_cron')!='immediate')
                {
					...
					elseif(QueueEmail($row->email,esc_html(stripslashes($_POST['subject'])),str_replace("<!--salutation-->",__('Dear','church-admin').' '.esc_html($row->first_name).',',$send_message),'',esc_html(stripslashes($_POST['from_name'])),esc_html(stripslashes($_POST['from_email'])),$attachments,$schedule)) echo'<p>'.esc_html($row->email).' queued</p>';
				}
				else
				{
					...
					if(!empty($row->email))
					{
						...
						if(defined('EMAIL_TEST')){church_admin_debug($row->email."\r\n");echo'<p>'.esc_html($row->email).' would have got it</p>';}
						elseif(wp_mail($row->email,esc_html(stripslashes($_POST['subject'])),str_replace('<!--salutation-->',__('Dear','church-admin').' '.$row->first_name.',',$send_message),NULL,$attachments)){echo'<p>'.esc_html($row->email).' sent immediately</p>';}
	...
	else
	{
		...
		echo'<tr><th scope="row">'.__('Send now','church-admin').' </th><td><input type=checkbox id="now" name=schedule value="now" checked="checked"/></td></tr>';
		echo'<tr><th scope="row">'.__('Or schedule?','church-admin').'</th><td><input name="send_date" id="send_date" type="text"  /></td></tr>';
