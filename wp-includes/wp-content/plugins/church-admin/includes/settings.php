	...
	if(!empty($_POST['save-email-settings']))
	{
		...
		if(!empty($_POST['cron']))
		{
			...
			switch($_POST['cron'])
			{
				...
				case 'cron':
								wp_clear_scheduled_hook('church_admin_bulk_email');
								echo'<p><a  target="_blank" href="'.site_url().'/?ca_download=cron-instructions&amp;cron-instructions='.wp_create_nonce('cron-instructions').'">'.__('PDF Instructions for email cron setup','church-admin').'</a></td></tr>';
				break;
				default:
								wp_clear_scheduled_hook('church_admin_bulk_email');
...
{
	if(empty($_POST))$sms_display='display:none';
	if(!empty($_POST['save-sms-settings'])&&!empty($_POST['church_admin_sms_username'])&&!empty($_POST['church_admin_sms_password'])&&!empty($_POST['church_admin_sms_iso'])&&!empty($_POST['church_admin_sms_type']))
	{
		update_option('church_admin_sms_username',$_POST['church_admin_sms_username']);
        update_option('church_admin_sms_password',$_POST['church_admin_sms_password']);
        update_option('church_admin_sms_reply',$_POST['church_admin_sms_reply']);
        update_option('church_admin_sms_iso',$_POST['church_admin_sms_iso']);
		update_option('church_admin_bulksms',$_POST['church_admin_sms_type']);
		//send test message
		$port = 80;
		...
		echo'<div class="ca-sms" ';
		if(!empty($sms_display)) echo 'style="'.$sms_display.'"';
		echo'>';
		...
		echo'<form action="admin.php?page=church_admin/index.php&amp;action=settings&amp;tab=settings" method="POST"><table class="form-table"><tbody><tr><th scope="row">Bulksms EAPI url</th><td><input type="text" name="church_admin_sms_type" required="required" ';
		if(!empty($sms_type)){ echo' value="'.esc_html($sms_type).'" ';}
		echo'/></td></tr>';
		echo'<tr><th scope="row" >'.__('SMS username','church-admin').'</th><td><input type="text" required="required" name="church_admin_sms_username" value="'.get_option('church_admin_sms_username').'" /></td></tr>';
		echo'<tr><th scope="row">'.__('SMS password','church-admin').'</th><td><input type="text" required="required"  name="church_admin_sms_password" value="'.get_option('church_admin_sms_password').'" /></td></tr>';
		echo'<tr><th scope="row" >'.__('SMS reply eg:447777123456','church-admin').'</th><td><input type="text" required="required"  name="church_admin_sms_reply" value="'.get_option('church_admin_sms_reply').'" /></td></tr>';
		echo'<tr><th scope="row" >'.__('Country code eg 44','church-admin').'</th><td><input type="text" required="required"  name="church_admin_sms_iso" value="'.get_option('church_admin_sms_iso').'" /></td></tr>';
		echo'<tr><th scope="row" >&nbsp;</th><td><input type="hidden" name="save-sms-settings" required="required"  value="1"/><input class="button-primary" type="submit"  value="'.__('Save Settings','church-admin').' &raquo;" /></td></tr></tbody></table></form>';

		echo'<script>jQuery(document).ready(function($) {$(".sms-settings").click(function(){$(".ca-sms").toggle();});});</script>';
		echo'</div><hr/>';
}
 ...
 */
 ...
 function church_admin_general_settings()
 {
	...
	if(!empty($_POST['save-general-settings']))
	{
		...
		if(!empty($_POST['prayer-request-message'])){update_option('church_admin_prayer_request_message',stripslashes($_POST['prayer-request-message']));}else{delete_option('church_admin_prayer_request_message');}
		...
		echo '/></td></tr>';
		echo'<tr><th scope="row">'.__('Message for Prayer request submission form','church-admin').'</th><td><textarea class="large-text" name="prayer-request-message">';
		$message=get_option('church_admin_prayer_request_message');
		if(!empty($message))echo esc_html($message);
		echo'</textarea></td></tr>';
		echo'<tr><th scope="row">'.__('GDPR message template ','church-admin').'</th><td>[SITE_URl],[CHURCH_NAME],[CONFIRM_URL],[EDIT_URL],[HOUSEHOLD_DETAILS]<br/><textarea class="large-text"  name="gdpr_template">';
		$gdprMessage=get_option('church_admin_gdpr_email');
		echo esc_html($gdprMessage);
		echo'</textarea></td></tr>';
		...
		echo '/></td></tr>';
		echo'<tr><th scope="row">'.__('Created user email message template ','church-admin').'</th><td>[SITE_URl],[USERNAME],[PASSWORD],[EDIT_PAGE]<br/><textarea class="large-text"  name="user_template">';
		...
		echo esc_html($userMessage);
		echo'</textarea></td></tr>';
		echo'<tr><th scope="row">'.__('Allow social media to be saved in directory','church-admin').'</th><td><input type="checkbox" name="socials" value="TRUE" ';
		$socials=get_option('church-admin-socials');
		if($socials) echo ' checked="checked" ';
		echo '/></td></tr>';
		...
		echo'<tr><th scope="row" >&nbsp;</th><td><input type="hidden" name="save-general-settings" value="1"/><input class="button-primary" type="submit"  value="'.__('Save Settings','church-admin').' &raquo;" /></td></tr></tbody></table></form></div>';
 ...
 function church_admin_marital_status()
 {
 		...
 		$out='<h2 class="marital-settings">'.__('Marital Status (Click to toggle)','church-admin').'</h2>';
 		$out.='<div class="ca-marital" style="display:none">';
 		...
 		$out.='<p><a class="button-primary"  href="'.wp_nonce_url('admin.php?page=church_admin/index.php&tab=settings&action=edit_marital_status','edit_marital_status').'">'.__('Add','church-admin').'</a></p>';
 		foreach($marital_status AS $key=>$value)
 		{
 			$out.='<tr>';
 			$edit='<a href="'.wp_nonce_url('admin.php?page=church_admin/index.php&tab=settings&action=edit_marital_status&amp;ID='.intval($key),'edit_marital_status').'">'.__('Edit','church-admin').'</a>';
 			$delete='<a onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&tab=settings&action=delete_marital_status&amp;ID='.intval($key),'delete_marital_status').'">'.__('Delete','church-admin').'</a>';
 			if($key==0){$out.='<td>&nbsp;</td><td>&nbsp;</td>';}else{$out.='<td>'.$edit.'</td><td>'.$delete.'</td>';}
 			$out.='<td>'.esc_html($value).'</td></tr>';
 		}
 		$out.='</tbody></table>';
 		$out.='</div>';
		$out.='<script>jQuery(document).ready(function($) {$(".marital-settings").click(function(){$(".ca-marital").toggle();});});</script><hr/>';
 ...
 function church_admin_delete_marital_status($ID)
 {
 	...
 	if(!empty($marital_status[$ID]))
 	{
 		...
 		$wpdb->query('UPDATE '.CA_PEO_TBL.' SET marital_status=0 WHERE marital_status="'.intval($ID).'"');
 ...
 function church_admin_people_types_list()
 {
	...
	$people_types=get_option('church_admin_people_type');
 	foreach($people_types AS $ID=>$type)
 	{
 		...
 		if($ID>2){$delete='<a onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&tab=settings&action=delete_people_type&amp;ID='.intval($ID),'delete_people_type').'">'.__('Delete','church-admin').'</a>';}else{$delete='&nbsp;';}
