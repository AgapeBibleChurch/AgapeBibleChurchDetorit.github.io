 ...
 */ 
	...
	$results=$wpdb->get_results('SELECT * FROM '.CA_PEO_TBL.' WHERE prayer_chain=1 ORDER BY last_name,first_name');
	if(!empty($results))
	{
	...
	else
	{
			...
			echo '<form action="" method="POST"><p><label>'.__('Add to Chain','church-admin').'</label><input type="text" name="name" placeholder="'.__('Name','church-admin').'"/><input type="hidden" name="add_to_prayer_chain"
