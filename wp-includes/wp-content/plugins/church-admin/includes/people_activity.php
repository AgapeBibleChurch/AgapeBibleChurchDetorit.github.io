...
function church_admin_recent_people_activity()
{
	...
	$limit = "LIMIT " . ($p->page - 1) * $p->limit  . ", " . $p->limit;
        ...
        if($results)
        {
            ...
            echo '</tbody><tfoot><tr><th>'.__('Edit','church-admin').'</th><th>'.__('Delete','church-admin').'</th><th>'.__('Name','church-admin').'</th><th>'.__('Member Level','church-admin').'</th><th>'.__('Follow Up Action','church-admin').'</th><th>'.__('Mobile','church-admin').'</th><th>'.__('Email','church-admin').'</th><th>'.__('Last Updated','church-admin').'</th></tr></tfoot></table>';
            // Pagination
...
if($results)
{
    ...
    foreach($results AS $row)
    {
        ...
        foreach($re AS $f_row)
        {
            ...
            $message.='</table>';
            $message.='<a href="'.admin_url().'?page=church_admin/index.php&amp;action=follow_up_completed&id='.md5('follow_up'.$f_row->id).'">'.__("Please click here to log completion",'church-admin').'</a>';
        }
