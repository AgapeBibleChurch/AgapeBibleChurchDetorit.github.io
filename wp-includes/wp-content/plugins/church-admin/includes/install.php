...
//working with children
    ...
    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_CP_TBL.' LIKE "application_form"')!='application_form')
    {
    	$wpdb->query('ALTER TABLE '.CA_CP_TBL.' ADD `application_form` TEXT NOT NULL AFTER `start_date`, ADD `reference1` TEXT NOT NULL AFTER `application_form`, ADD `reference2` TEXT NOT NULL AFTER `reference1`, ADD `interview` TEXT NOT NULL AFTER `reference2`');
	}
...
if($church_admin_version>=0.943){
	...
	if(empty($modules))
	{
		$modules=array('App'=>TRUE,'People'=>TRUE,'Sessions'=>TRUE,'Services'=>TRUE,'Attendance'=>TRUE,'Podcast'=>TRUE,'Rota'=>TRUE,'Children'=>TRUE,'Classes'=>TRUE,'Calendar'=>TRUE,'Comms'=>TRUE,'Groups'=>TRUE,'Media'=>TRUE,'Facilities'=>TRUE,'Ministries'=>TRUE);
		update_option('church_admin_modules',$modules);
	}
  if(empty($modules['Attendance'])){$modules['Attendance']=TRUE;update_option('church_admin_modules',$modules);}
    ...
    {
        $sql='CREATE TABLE  '.CA_SERM_TBL.' (`series_name` TEXT NOT NULL ,`series_image` TEXT NOT NULL,`series_description` TEXT NOT NULL ,last_sermon DATETIME,`series_id` INT( 11 ) NOT NULL AUTO_INCREMENT PRIMARY KEY) ENGINE = MYISAM CHARACTER SET utf8 COLLATE utf8_general_ci;';
        $wpdb->query($sql);
    }
    //update from 1.140
    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_SERM_TBL.' LIKE "last_sermon"')!='last_sermon')
{
    $sql='ALTER TABLE  '.CA_SERM_TBL.' ADD last_sermon DATETIME';
    $wpdb->query($sql);
    //go thorugh each series to find last date
    $sql='SELECT max(pub_date) AS pub_date, series_id FROM '.CA_FIL_TBL.' GROUP BY series_id ORDER BY pub_date DESC';
	$results=$wpdb->get_results($sql);
	if(!empty($results))
	{
		foreach($results AS $row)
		{
			$wpdb->query('UPDATE '.CA_SERM_TBL.' SET last_sermon="'.$row->pub_date.'" WHERE series_id="'.intval($row->series_id).'"');
		}
	}
}
/*********************************************************
...
if($wpdb->get_var('SHOW COLUMNS FROM '.CA_FIL_TBL.' LIKE "email_sent"')!='email_sent')
{
   $sql='ALTER TABLE '.CA_FIL_TBL.' ADD `email_sent` DATE NOT NULL AFTER `external_file`;';
   $wpdb->query($sql);
}
...
if ($wpdb->get_var('SHOW TABLES LIKE "'.CA_HOP_TBL.'"') != CA_HOP_TBL)
{
    ...
    {
        $sql = 'CREATE TABLE '.CA_HOU_TBL.' ( privacy INT(1) DEFAULT 0,address TEXT, lat VARCHAR(50),lng VARCHAR (50), phone VARCHAR(15),member_type_id INT(11),ts timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,household_id int(11) NOT NULL AUTO_INCREMENT, PRIMARY KEY (household_id));';
        $wpdb->query($sql);
    }
    //people table
    ;
    ...
    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "sms_send"')!='sms_send')
    {
    $sql='ALTER TABLE  '.CA_PEO_TBL.' ADD sms_send INT(1) DEFAULT 1';
    $wpdb->query($sql);

     }
	if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "mail_send"')!='mail_send')
    {
    $sql='ALTER TABLE  '.CA_PEO_TBL.' ADD mail_send INT(1) DEFAULT 1';
    $wpdb->query($sql);

     }
    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "gdpr_reason"')!='gdpr_reason')
    {
    $sql='ALTER TABLE  '.CA_PEO_TBL.' ADD gdpr_reason TEXT';
    $wpdb->query($sql);
...
$wpdb->query('UPDATE '.CA_MET_TBL.' SET meta_type="class" WHERE meta_type="classes"');
...
if ($wpdb->get_var('SHOW TABLES LIKE "'.CA_MIN_TBL.'"') != CA_MIN_TBL)
    {
        ...
        {
        	$sql='INSERT INTO '.CA_MIN_TBL.' (ministry,ID) VALUES ("'.__('Small Group Leader','church-admin').'",1),("'.__('Elder','church-admin').'",2),("'.__('Prayer requests send','church-admin').'",3),("'.__('Bible readings send','church-admin').'",4)';
			$wpdb->query($sql);
        }
...
/********************************************************************************
*
*   from version 1.2608 Prayer Requests Send and Bible Readings send are now ministries
*
*********************************************************************************/
  if($wpdb->get_var('SHOW COLUMNS FROM '.CA_MIN_TBL.' LIKE "mailchimp_id"')!='mailchimp_id')
  {//update ministry table to add in mailchimp id for segment
    $sql='ALTER TABLE  '.CA_MIN_TBL.' ADD mailchimp_id TEXT ';
    $wpdb->query($sql);

  }
    $prayer_id=$wpdb->get_var('SELECT ID FROM '.CA_MIN_TBL.' WHERE ministry="'.__('Prayer requests send','church-admin').'"');
    if(empty($prayer_id))
    {
      $wpdb->query('INSERT INTO '.CA_MIN_TBL.' (ministry) VALUES ("'.__('Prayer requests send','church-admin').'") ');
      $prayer_id=$wpdb->insert_id;
    }
    //add people with prayer_chain status to that ministry
    $people=$wpdb->get_results('SELECT people_id FROM '.CA_PEO_TBL.' WHERE prayer_chain=1');
    if(!empty($people))
    {
        foreach($people AS $person)church_admin_update_people_meta($prayer_id,$person->people_id,'ministry');

    }
    $bible_id=$wpdb->get_var('SELECT ID FROM '.CA_MIN_TBL.' WHERE ministry="'.__('Bible readings send','church-admin').'"');
    if(empty($bible_id))
    {
        $wpdb->query('INSERT INTO '.CA_MIN_TBL.' (ministry) VALUES ("'.__('Bible readings send','church-admin').'") ');
        $bible_id=$wpdb->insert_id;
    }
    if(!empty($people))
    {
        foreach($people AS $person)church_admin_update_people_meta($bible_id,$person->people_id,'ministry');

    }
    $news_id=$wpdb->get_var('SELECT ID FROM '.CA_MIN_TBL.' WHERE ministry="'.__('News send','church-admin').'"');
    if(empty($news_id))
    {
        $wpdb->query('INSERT INTO '.CA_MIN_TBL.' (ministry) VALUES ("'.__('News send','church-admin').'") ');
        $news_id=$wpdb->insert_id;
    }
    if(!empty($people))
    {
        foreach($people AS $person)church_admin_update_people_meta($news_id,$person->people_id,'ministry');

    }

    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_MIN_TBL.' LIKE "parentID"')!='parentID')
    {//update ministry table to make more flexible looking!
...
if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "smallgroup_id"')=='smallgroup_id')
{
	$check=$wpdb->get_var('SELECT COUNT(people_id) FROM '.CA_MET_TBL.' WHERE meta_type="smallgroup"');
	if(empty($check))
	{
	...
	if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "smallgroup_id"')=='smallgroup_id')$wpdb->query('ALTER TABLE '.CA_PEO_TBL.' DROP COLUMN smallgroup_id');
	if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "smallgroup_attendance"')=='smallgroup_attendance')$wpdb->query('ALTER TABLE '.CA_PEO_TBL.' DROP COLUMN smallgroup_attendance');
	if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "departments"')=='departments')$wpdb->query('ALTER TABLE '.CA_PEO_TBL.' DROP COLUMN departments');
...
//migrate old tables
	...
	foreach($results AS $row)
	{
	    ...
	    $member_data=esc_sql(serialize(array('1'=>mysql2date('Y-m-d',$row->ts))));
	...
	foreach($results AS $row)
	{
		...
		foreach($adults AS $key=>$adult)
		{
			...
			{//update member data
			    ...
			    $member_data['2']=$row->first_sunday;
...
*
*
* Update member data column of peo table for 1.6200
*
*
******************/
$db_version=get_option('church_admin_DB_version');
if(empty($db_version) || $db_version<1.1620)
{
	$member_type=church_admin_member_type_array();


	$people=$wpdb->get_results('SELECT people_id,member_data FROM '.CA_PEO_TBL);
	if(!empty($people))
	{
		foreach($people AS $person)
		{
			church_admin_debug('<p> Person - '.$person->people_id);
			$newMemberData=array();
			$member_data=maybe_unserialize($person->member_data);//persons member data

			if(!empty($member_data))
			{
				foreach($member_data AS $oldkey=>$value)
				{
					$deSanitizedOldKey=str_replace('_',' ',$oldkey); //old keys had _ instead of spaces for name of member type

					$key=array_search($deSanitizedOldKey,$member_type);//get the key number for member_type
					if(!empty($key))
					{
						if(!empty($value))$newMemberData[$key]=$value;//set new data

					}
					else
					{
						church_admin_debug($deSanitizedOldKey.' not found');
					}
				}
				$sql='UPDATE '.CA_PEO_TBL.' SET member_data="'.esc_sql(serialize($newMemberData)).'" WHERE people_id="'.intval($person->people_id).'"';

				$wpdb->query($sql);
			}
		}
	}
	update_option('church_admin_DB_version',1.1620);
}
...
{
	$wpdb->query('ALTER TABLE  '.CA_PEO_TBL.' ADD site_id INT(11) NULL DEFAULT "1"');
}
$wpdb->query('UPDATE '.CA_PEO_TBL.' SET site_id=1 WHERE site_id IS NULL');
...
//fix bug in 1.3600
$peopleCount=$wpdb->get_var('SELECT COUNT(*) FROM '.CA_PEO_TBL);
$head=$wpdb->get_var('SELECT COUNT(*) FROM '.CA_PEO_TBL.' WHERE head_of_household=1');
if($peopleCount==$head)$wpdb->query('UPDATE '.CA_PEO_TBL.' SET head_of_household=0');
//make sure every household has a head of household...
$households=$wpdb->get_results('SELECT * FROM '.CA_HOU_TBL);
	if(!empty($households))
	{
		foreach($households AS $household)
		{
        	...
        	$sql='UPDATE '.CA_PEO_TBL.' SET head_of_household=1 WHERE household_id="'.esc_sql($household->household_id).'" AND people_id="'.esc_sql($people[0]->people_id).'"';
					...
					$wpdb->query($sql);
          $sql='UPDATE '.CA_PEO_TBL.' SET head_of_household=0 WHERE household_id="'.esc_sql($household->household_id).'" AND people_id!="'.esc_sql($people[0]->people_id).'"';
					$wpdb->query($sql);
				}
			}
		}
	...
	$sql="CREATE TABLE  ". $table_name ." (leadership TEXT NOT NULL,group_name varchar(255) NOT NULL,whenwhere TEXT NOT NULL,address TEXT, lat VARCHAR(30),lng VARCHAR(30), id int(11) NOT NULL AUTO_INCREMENT,PRIMARY KEY (id));";
        $wpdb->query($sql);
	$wpdb->query('INSERT INTO '.CA_SMG_TBL.' (group_name,id)VALUES ( "'.__('Unattached','church-admin').'", "1");');
    }
...
//services
	...
	//fix service_day going less than 0 in v1.2490
	$wpdb->query('UPDATE '.CA_SER_TBL.' SET service_day=0 WHERE service_day<0');
...
if($wpdb->get_var('SHOW COLUMNS FROM '.CA_CLA_TBL.' LIKE "end_date"')!='end_date')
	{
		$sql='ALTER TABLE  '.CA_CLA_TBL.' ADD	`end_date` DATE  AFTER `end_time`';
		$wpdb->query($sql);
	}
		if($wpdb->get_var('SHOW COLUMNS FROM '.CA_CLA_TBL.' LIKE "leadership"')!='leadership')
	{
		$sql='ALTER TABLE  '.CA_CLA_TBL.' ADD	`leadership` TEXT  AFTER `end_time`';
		$wpdb->query($sql);
	}
			if($wpdb->get_var('SHOW COLUMNS FROM '.CA_CLA_TBL.' LIKE "cat_id"')!='cat_id')
	{
		$sql='ALTER TABLE  '.CA_CLA_TBL.' ADD	`cat_id` INT(11)  AFTER `leadership`';
		$wpdb->query($sql);
	}
...
if(!empty($classes))
{
	foreach($classes AS $class)
	{
		...
		if(!is_array($ldrs))
		{
			...
			if(is_array($ldrs))$wpdb->query('UPDATE '.CA_CLA_TBL.' SET leadership="'.esc_sql(maybe_serialize($ldrs)).'" WHERE class_id="'.intval($class->class_id).'"');
		}
...
//events, tickets and bookings
    ...
    if($wpdb->get_var('show tables like "'.CA_EVE_TBL.'"') != CA_EVE_TBL)
    {
        $sql="CREATE TABLE IF NOT EXISTS ". CA_EVE_TBL." (name TEXT NOT NULL,  description TEXT NOT NULL,  event_date DATE NOT NULL,pageID INT(11), event_id INT(11) NOT NULL AUTO_INCREMENT, PRIMARY KEY (event_id));";
        $wpdb->query($sql);
    }
    if($wpdb->get_var('SHOW COLUMNS FROM '.CA_EVE_TBL.' LIKE "pageID"')!='pageID')
  {
    $sql='ALTER TABLE  '.CA_EVE_TBL.' ADD `pageID` INT(11) AFTER event_date';
    $wpdb->query($sql);
  }
    if($wpdb->get_var('show tables like "'.CA_TIK_TBL.'"') != CA_TIK_TBL)
    {
        $sql="CREATE TABLE IF NOT EXISTS ". CA_TIK_TBL." (name TEXT NOT NULL,  description TEXT NOT NULL,  available_from DATE NOT NULL,available_until DATE NOT NULL, quantity INT(11), price FLOAT(6,2), event_id INT(11), ticket_id INT(11) NOT NULL AUTO_INCREMENT, PRIMARY KEY (ticket_id));";
        $wpdb->query($sql);
    }
    if($wpdb->get_var('show tables like "'.CA_BOO_TBL.'"') != CA_BOO_TBL)
    {
        $sql="CREATE TABLE IF NOT EXISTS ". CA_BOO_TBL." (event_id INT(11) NOT NULL,  tickets TEXT NOT NULL,  payment_amount FLOAT(6,2) , people_id INT(11), booking_date DATE NOT NULL, booking_status TEXT NOT NULL,ticket_id INT(11) AUTO_INCREMENT, PRIMARY KEY (ticket_id));";
        $wpdb->query($sql);
    }
/*******************************************************************
*
...
*
*******************************************************************/
    if($wpdb->get_var('show tables like "'.CA_CUST_TBL.'"') != CA_CUST_TBL)
    {

    	$sql='CREATE TABLE '.CA_CUST_TBL.' (`people_id` INT(11), `data` TEXT, `custom_id` INT(11),`ID` INT(11) AUTO_INCREMENT, PRIMARY KEY (ID));';
    	$wpdb->query($sql);
  ...
  if($wpdb->get_var('SHOW COLUMNS FROM '.CA_DATE_TBL.' LIKE "link"')!='link')
  {
      $sql='ALTER TABLE  '.CA_DATE_TBL.' ADD link TEXT AFTER facilities_id';
      $wpdb->query($sql);
  }
  if($wpdb->get_var('SHOW COLUMNS FROM '.CA_DATE_TBL.' LIKE "link_title"')!='link_title')
  {
      $sql='ALTER TABLE  '.CA_DATE_TBL.' ADD link_title TEXT AFTER link';
      $wpdb->query($sql);
  }
  ...
  {
    if( $wpdb->get_var('SHOW COLUMNS FROM '.CA_HOU_TBL.' LIKE "privacy"')!='privacy')
	   {
		     $sql='ALTER TABLE  '.CA_HOU_TBL.' CHANGE `private` `privacy` INT( 1 ) DEFAULT 0 ';
		       $wpdb->query($sql);
    }
  }
  if($wpdb->get_var('SHOW COLUMNS FROM '.CA_HOU_TBL.' LIKE "private"')=='private')
  {
    $sql='ALTER TABLE  '.CA_HOU_TBL.' DROP `private`';
		$wpdb->query($sql);
	}
		if($wpdb->get_var('SHOW COLUMNS FROM '.CA_HOU_TBL.' LIKE "attachment_id"')!='attachment_id')
	{
		$sql='ALTER TABLE  '.CA_HOU_TBL.' ADD `attachment_id` INT (11) AFTER ts ';
		$wpdb->query($sql);
	}
...
if($wpdb->get_var('SHOW COLUMNS FROM '.CA_SMG_TBL.' LIKE "group_day"')!='group_day')
{
    $sql='ALTER TABLE  '.CA_SMG_TBL.' ADD group_day INT(1)';
    $wpdb->query($sql);
    $sql='ALTER TABLE  '.CA_SMG_TBL.' ADD group_time TIME';
    $wpdb->query($sql);
}
...
/*
if ($wpdb->get_var('SHOW TABLES LIKE "'.CA_METRICS_TBL.'"') != CA_METRICS_TBL)
{

	 $sql = 'CREATE TABLE IF NOT EXISTS '.CA_METRICS_TBL.' (  `metric` text NOT NULL,  `ID` int(11) NOT NULL AUTO_INCREMENT,  PRIMARY KEY (`ID`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1';
	 $wpdb->query($sql);
   $sql='INSERT INTO '.CA_METRICS_TBL.'(metric) VALUES("'.__('Average adult attedance','church-admin').'"),("'.__('Average child attendance','church-admin').'"),("'.__('Peak adult attendance','church-admin').'"),("'.__('Peak child attendance','church-admin').'"),("'.__('First time guests','church-admin').'"),("'.__('Second time guests','church-admin').'"),("'.__('Salvations','church-admin').'"),("'.__('Baptisms','church-admin').'"),("'.__('Giving','church-admin').'"),("'.__('Volunteers','church-admin').'")';
   $wpdb->query($sql);
}

if ($wpdb->get_var('SHOW TABLES LIKE "'.CA_METRICS_META_TBL.'"') != CA_METRICS_META_TBL)
{

	 $sql = 'CREATE TABLE IF NOT EXISTS '.CA_METRICS_META_TBL.' (  `metric_id` INT(11) NOT NULL,service_id INT(11) NOT NULL, metric_date DATE NOT NULL,metric_data TEXT,  `ID` int(11) NOT NULL AUTO_INCREMENT,  PRIMARY KEY (`ID`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1';
	 $wpdb->query($sql);
}
*/
...
//update rota email cron so message is attached if not already.
$cronJobs=get_option('cron');
if(!empty($cronJobs))
{
  foreach($cronJobs AS $ts=>$details)
  {
    if(is_array($details)&&array_key_exists('church_admin_cron_email_rota',$details))
    {
        $cronJobs=$details['church_admin_cron_email_rota'];
        foreach($cronJobs AS $key=>$cronJob)
        {
          if(empty($cronJobs['args']['message']))$cronjobs[$ts][$key]['args']['message']=get_option('church_admin_auto_rota_email_message');
        }
    }

  }
  update_option('cron',$cronJobs);
}
...
if($wpdb->get_var('SHOW COLUMNS FROM '.CA_FIL_TBL.' LIKE "bible_texts"')!='bible_texts')
	{
    	$sql='ALTER TABLE  '.CA_FIL_TBL.' ADD bible_texts TEXT AFTER `bible_passages`';
    	$wpdb->query($sql);
	}
...
if(empty($check))
{
 ...
 $gdpr=array(
 				__('Subscribed via Mailchimp','church-admin'),
 				__('User registered on the website','church-admin'),
 				__('User confirmed using GDPR form','church-admin'),
 				__('Verbal confirmation','church-admin')
 			);
...
if(empty($gdprMessage))
{
    $gdprMessage='<p>There are some new data protection regulations coming in to protect how your personal data is used. We, [CHURCH_NAME], store your name, address and phone details so we can keep the church organised and would like to be able to continue to communicate by email, sms and mail with you. Your contact details are available on the website [SITE_URL] within a password protected area. Please check with other members of your household who are over 16 and click this [CONFIRM_URL] if you are happy. If you are not happy or would like to discuss further then do get in touch with the church office.</p><p>[EDIT_URL]</p>[HOUSEHOLD_DETAILS]';
    update_option('church_admin_gdpr_email',$gdprMessage);
}

  //put member data in people_meta table
  if($wpdb->get_var('SHOW COLUMNS FROM '.CA_PEO_TBL.' LIKE "member_data"')=='member_data')
  {
    church_admin_debug('Sorting member data');;
    $sql='SELECT people_id, member_data FROM '.CA_PEO_TBL;
  ...
  if(!empty($people))
  {
    ...
    foreach($people AS $peep)
    {
        ...
        if(!empty($member_data))
        {
          ...
          {
            $data[]='("'.intval($peep->people_id).'","member_date","'.intval($id).'","'.esc_sql($date).'")';
          }
        }
    }
        church_admin_debug(print_r($data,TRUE));
        if(!empty($data))$wpdb->query('INSERT INTO '.CA_MET_TBL.' (people_id,meta_type,ID,meta_date) VALUES '.implode(",",$data));
        $wpdb->query('ALTER TABLE '.CA_PEO_TBL.' DROP member_data;');

  }

}//end of adjust member_data
