...
function church_admin_send_sms()
{
	...
	{
		echo'<h2>'.__('Please setup your Bulksms account settings first','church-admin').'</h2>'; // Translating added by JF 8.02.18
	}
	else
	{
	...
	echo'<p><strong>'.__('You need some people in the directory before you can use this Bulk SMS service','church-admin').'</strong></p>';  // Translating added by JF 8.02.18
		...
		if(!empty($_POST['recipients']))
	{
				...
				$sql='SELECT  mobile,first_name FROM '.CA_PEO_TBL.' WHERE mobile!="" AND '.implode(' OR ',$names).' AND sms_send=1 GROUP BY mobile';
	}
	else
	{
		$group_by='';
	...
	$sql='SELECT a.first_name, a.mobile AS mobile, a.people_id FROM '.implode(", ",array_filter($tables)).' WHERE a.household_id=b.household_id '.$other.' AND a.mobile!="" AND a.sms_send=1 AND gdpr_reason!="" GROUP BY a.mobile  ORDER BY a.last_name';
}
            ...
            foreach ($results AS $row)
                {
                    ...
                    $country=get_option('church_admin_sms_iso');
					$sendmobile=$country.$mobile;//non UK
					if(($country==44||$country==33) && !empty($mobile) && $mobile{0}=='0')//uk & FR remove preceding 0
					{
             			$sendmobile=$country.substr($mobile, 1);
        			}
...
<h1>'.__('Send a text message','church-admin').'</h1>
<form action="" method="post" name="SMS" id="SMS">
<p><textarea class="sms" id="counttxt" rows="4" cols="50" name="counttxt"  onkeyup="counterUpdate(\'counttxt\', \'countBody\',\'160\');"></textarea> <label>'.__('Message','church-admin').' <span id="countBody">&nbsp;&nbsp;0</span>/160 '.__('characters','church-admin').'</label></p>'; // Translating added by JF 8.02.18 And moved the label to the right of the text area?

if ( function_exists('wp_nonce_field') )wp_nonce_field('church admin send sms');
	echo'<h2>'.__('Choose recipients...','church-admin').'</h2>';
	$smsoremail='mobile';
	$member_type=church_admin_member_type_array();
	...
	echo'<p>'.__('Or use the filters below. ','church-admin').'<span id="filtered-response">'.__('Everyone will get this, unless you add some filters','church-admin').'</span></p>'; // Added: period and space after filters below JF 8.02.18
...
//end of choose recipients
	...
	echo'<p><br style="clear:left"/><input class="button-primary" type="submit" name="submitted" value="'.__('Send Message','church-admin').'"/></p></form>';  // Translation JF 8.02.18
}
...
function formatted_server_response( $result ) {
  ...
  if ($result['success']) {
    ...
    if ($result['transient_error']) {
      $this_result .=  "This is a transient error - you should retry it in a production environment";  // Translation? JF 8.02.18
    }
  }
  return $this_result;
}
