...
function church_admin_small_groups()
{
	...
	global $wpdb,$current_user,$people_type,$wp_locale;
	wp_get_current_user();
	...
	*****************************/
	...
	$out.= '<form name="smallgroup_form" action="'.home_url().'" method="get"><table class="form-table"><input type="hidden" name="ca_download" value="smallgroup"/>';
		...
		foreach ($sg_results as $sg_row)
		{
			...
			{//only build row if user allowed to see it
    			...
    			foreach($hierarchy AS $key=>$min_id)
    			{
    				...
    				if(!empty($curr_leaders[$min_id])){foreach($curr_leaders[$min_id] AS $k=>$people_id)$ldr.='<span class="ca-names">'.esc_html(church_admin_get_person($people_id)).'</span><br/>';}else{$ldr.='No leaders assigned yet<br/>';}
    			}
    			$ldr.='</p>';
        		...
        		if($sg_row->id!=1)
				{
					...
					church_admin_debug('Group day: '.$sg_row->group_day);
					$out.='<tr class="sortable-row" id="'.$sg_row->id.'"><td><a href="'.wp_nonce_url($edit_url, 'edit_small_group').'">'.__('Edit','church-admin').'</a></td><td><a onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url($delete_url, 'delete_small_group').'">'.__('Delete','church-admin').'</a></td><td class="ca-names"><a title="'.__('Who is in this group?','church-admin').'" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;action=whosin&amp;id='.intval($sg_row->id),'whosin').'">'.esc_html(stripslashes($sg_row->group_name)).'</a></td><td class="ca-names">'.$ldr.'</td><td>'.$wp_locale->get_weekday($sg_row->group_day).' '.$sg_row->group_time.'</td></tr>';
				}
				else
				{
					if(empty($sg_row->group_day))$sg_row->group_day=1;
					$out.='<tr class="sortable-row" id="'.intval($sg_row->id).'"><td>&nbsp;</td><td>&nbsp;</td><td >'.esc_html(stripslashes($sg_row->group_name)).'</td><td>&nbsp;</td><td>'.$wp_locale->get_weekday($sg_row->group_day).' '.$sg_row->group_time.'</td></tr>';
       			}
...
function church_admin_edit_small_group($id)
{
    ...
    if(isset($_POST['edit_small_group']))
    {
		...
		if($id)
		{//update
			$sql='UPDATE '.CA_SMG_TBL.' SET lat="'.esc_sql($form['lat']).'",lng="'.esc_sql($form['lng']).'",leadership="'.esc_sql(serialize($leaders)).'",address="'.esc_sql($form['address']).'",group_name="'.esc_sql($form['group_name']).'",group_day="'.intval($form['group_day']).'",group_time="'.esc_sql($form['group_time']).'" WHERE id="'.esc_sql(intval($id)).'"';
		...
		{//insert
			$sql='INSERT INTO  '.CA_SMG_TBL.' (group_name,group_day,group_time,address,lat,lng,leadership) VALUES("'.esc_sql($form['group_name']).'","'.esc_sql($form['group_day']).'","'.esc_sql($form['group_time']).'","'.esc_sql($form['address']).'","'.esc_sql($form['lat']).'","'.esc_sql($form['lng']).'","'.esc_sql(serialize($leaders)).'")';
    ...
    else
    {
	    ...
	    $out.='/></td></tr>';
	    if(!empty($data->whenwhere))$out.='<tr><th scope="row"><strong>'.__('Old Value','church-admin').'</th><td> '.esc_html($data->whenwhere).'</td></tr>';
	   	$out.='<tr><th scope="row">'.__('Meeting Day','church-admin').'</th><td><select name="group_day"> ';
       	$myweek = array();

		for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
			church_admin_debug('Begins: '.( $wdcount + $week_begins ) % 7 );
			$myweek[] = $wp_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
		}
		$first=$option='';
		foreach ( $myweek as $key=>$wd ) {
		church_admin_debug('wd: '.$wd);
			$day_name = $initial ? $wp_locale->get_weekday_initial( $wd ) : $wp_locale->get_weekday_abbrev( $wd );
			$wd = esc_attr( $wd );

			if(!empty($data->group_day)&&$key==$data->group_day)
			{
				$first='<option value="'.intval($key).'" selected="selected">'.$day_name.'</option>';

			}
			else
			{
				$option.='<option value="'.intval($key).'">'.$day_name.'</option>';
			}

		}
		$out.=$first.$option;
       $out.='</select></td></tr>';
	    $out.='<tr><th scope="row">'.__('Start Time','church-admin').'</th><td><input type="text" id="group_time" name="group_time"';
    	...
    	foreach($hierarchy AS $key=>$min_id)
    	{
    		...
    		$out.='<tr class="ca-types autocomplete"><th scope=row>'.__('Leaders for','church-admin').' "' .esc_html($ministries[$min_id]).'"</th><td class="ca-names">'.church_admin_autocomplete('a'.intval($min_id),'a'.intval($min_id),'a'.intval($min_id),$current).'</td></tr>';
