...
{
	global $wpdb;
	$out='<h1 class="kidswork">'.__('Kids Work (Click to view)','church-admin').'</h1><div id="kidswork-section" style="display:none">';
	...
	$out.= '<form name="kidswork_form" action="'.home_url().'" method="get"><input type="hidden" name="ca_download" value="kidswork_pdf"/>';
	...
	$out.='</div>';
		$out.='<script type="text/javascript">jQuery(function(){  jQuery(".kidswork").click(function(){jQuery("#kidswork-section").toggle();  });});</script>';
return $out;
}
...
function church_admin_edit_kidswork($id=NULL)
{
	...
	else
	{
		...
		echo'<tr><th scope="row">'.__('Youngest','church-admin').'</th><td>'.church_admin_date_picker($youngest,'youngest',FALSE,1910,date('Y'),'youngest','youngest').'</td></tr>';
		if(!empty($data->oldest)){$oldest=$data->oldest;}else{$oldest=NULL;}
		echo'<tr><th scope="row">'.__('Oldest','church-admin').'</th><td>'.church_admin_date_picker($oldest,'oldest',FALSE,1910,date('Y'),'oldest','oldest').'</td></tr>';
...
function church_admin_safeguarding_main()
{
	...
	$out.='<div id="safeguarding" style="display:none">';
	...
	$out.='</div>';
	$out.='<script type="text/javascript">jQuery(function(){  jQuery(".safeguarding-toggle").click(function(){console.log("Toggle safeguarding");jQuery("#safeguarding").toggle();  });});</script>';
...
function church_admin_safeguarding_list()
{
	...
	$sql='SELECT CONCAT_WS(" ",a.first_name,a.prefix,a.last_name) AS name,a.active,b.* FROM '.CA_PEO_TBL.' a, '.CA_CP_TBL.' b WHERE b.people_id=a.people_id ORDER BY a.last_name,a.first_name';
...
function church_admin_uk_safeguarding_table($results)
{
		...
		<thead>
			<tr>
				...
				<th>'.__('Application form','church-admin').'</th>
				<th>'.__('Reference 1','church-admin').'</th>
				<th>'.__('Reference 2','church-admin').'</th>
				<th>'.__('Interview','church-admin').'</th>
				<th>'.__('DBS Status and Action','church-admin').'</th>
				<th>'.'DBS Number'.'</th>
				<th>'.'DBS Date?'.'</th>
		...
		{//build table
			...
			<tr '.$class.'>
				<td '.$class.'>'.$edit.'</td>
				<td '.$class.' class="ca-names">'.esc_html($row->name).'</td>
				<td '.$class.'>'.esc_html(implode(", ",$mins)).'</td>
				<td '.$class.'>'.esc_html($row->employment_status).'</td>
				<td '.$class.'>'.mysql2date(get_option('date_format'),$row->start_date).'</td>
				<td '.$class.'>'.esc_html($row->application_form).'</td>
				<td '.$class.'>'.esc_html($row->reference1).'</td>
				<td '.$class.'>'.esc_html($row->reference2).'</td>
				<td '.$class.'>'.mysql2date(get_option('date_format'),$row->interview).'</td>
				<td '.$class.'>'.esc_html($row->status).'</td>
				<td '.$class.'>'.esc_html($row->DBS).'</td>
				<td '.$class.'>'.mysql2date(get_option('date_format'),$row->DBS_date).'</td>
				<td '.$class.'>'.mysql2date(get_option('date_format'),$row->review_date).'</td>
			</tr>';
		}//build table
		$out.='</tbody></table>';
	...
	switch($nation)
	{
	 	...
	 	case 'United Kingdom':
	 		...
	 		$fields=array(
	 			...
	 			'application_form'=>__('Application Form','church-admin'),
				'reference1'=>__('Reference 1','church-admin'),
	 			'reference2'=>__('Reference 2','church-admin'),
				'interview'=>__('Interview','church-admin'),
				'status'=>__('DBS Status & Action','church-admin'),
	 			'DBS'=>'DBS Number',
	 			'DBS_date'=>'DBS date',
	 			'review_date'=>__('Review Date','church-admin')
			);
		...
		foreach($fields AS $col=>$title)
		{
			if($col!='people_id')
			{
				...
				switch($col)
				{
					...
					case 'DBS_date':
					case 'interview':
						$out.=church_admin_date_picker($row->$col,$col,FALSE,date('Y',strtotime('-20 years')),NULL,sanitize_title($col),sanitize_title($col));
					break;
