	...
	if(isset($_POST['rota_task'])&&check_admin_referer('edit_rota_settings'))
	{
    	...
    	{//insert
        	...
        	if(!$id)
        	{
            	...
            	$sql='INSERT INTO '.CA_RST_TBL.' (rota_task,initials,rota_order,service_id,ministries) VALUES("'.$rota_task.'","'.$initials.'","'.esc_sql($rota_order).'","'.esc_sql(serialize($services)).'","'.esc_sql($newMinistries).'")';

            	$wpdb->query($sql);
            	$job_id=$wpdb->insert_id;


            	if(!empty($job_id)){echo'<div id="message" class="notice notice-success inline"><p><strong>'.__('Schedule Job Added','church-admin').'</strong></p></div>';}else{{echo'<div id="message" class="notice notice-success inline"><p><strong>'.__('Schedule Job failed to save','church-admin').'</strong></p></div>';}}
            	church_admin_rota_settings_list();
        	}else
        	{
            	$sql='UPDATE '.CA_RST_TBL.' SET rota_task="'.esc_sql(stripslashes($_POST['rota_task'])).'",service_id="'.esc_sql(serialize($services)).'",initials="'.$initials.'" WHERE rota_id="'.esc_sql($id).'"';
    	...
    	{//update
        	$sql='UPDATE '.CA_RST_TBL.' SET rota_task="'.esc_sql(stripslashes($_POST['rota_task'])).'",service_id="'.esc_sql(serialize($services)).'",initials="'.$initials.'",ministries="'.esc_sql($newMinistries).'" WHERE rota_id="'.esc_sql($id).'"';
...
echo '<p><a href="'.wp_nonce_url("admin.php?page=church_admin/index.php&amp;action=church_admin_edit_rota_settings",'edit_rota_settings').'">'.__('Add a schedule job','church-admin').'</a></p>';
echo'<p>'.__('Schedule tasks can be sorted by drag and drop, for use in other parts of the plugin.','church-admin').'</p>';
