...
function church_admin_recent_display($month)
{
    ...
    {
        $sql='SELECT a.*,b.* FROM '.CA_PEO_TBL.' a, '.CA_HOU_TBL.' b WHERE a.household_id=b.household_id AND a.last_updated>DATE_SUB(NOW(), INTERVAL '.esc_sql($month).' MONTH) AND a.member_type_id ="'.esc_sql($type_id).'"';
	...
	if(!empty($results))
	{
		...
		foreach($recent_dates AS $service_date=>$people)
		{
			...
			foreach($people AS $person)
			{
				...
				$out.='<p><a title="Edit person" href="'.wp_nonce_url('admin.php?page=church-admin/index.php&amp;action=edit_people&amp;people_id='.intval($person['people_id']),'edit_people').'">'.esc_html($person['first_name']).'</a> ';
				$out.='<a href="'.wp_nonce_url('admin.php?page=church-admin/index.php&amp;action=edit_household&amp;household_id='.intval($person['household_id']),'edit_household').'" title="edit household">';
				if(!empty($use_prefix)&&!empty($person['prefix']))$out.=esc_html($person['prefix']).' ';
				$out.=esc_html($person['last_name']).'</a></p>';
