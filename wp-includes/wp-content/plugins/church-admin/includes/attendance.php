...
function church_admin_attendance_list()
{
	...
	echo'<input type="hidden" name="page" value="church_admin/index.php"/><input type="hidden" name="action" value="attendance"/><input type="hidden" name="tab" value="attendance"/>';
...
function church_admin_edit_attendance($attendance_id)
{
  global $wpdb,$wp_locale;
	...
	{//safe to proceed
		...
		echo '<tr><th scope="row">'.__('Date','church-admin').' :</th><td>'.church_admin_date_picker($date,'add_date',FALSE,'2011',date('Y',time()+60*60*24*365*10),'add_date','add_date').'</td></tr>';
...
else
{
     ...
     foreach($services AS $service)
     {
	  ...
	  if($service->service_id==$service_id)$service_details=sprintf( __('%1$s on %2$s at %3$s', 'church-admin'), esc_html($service->service_name),$wp_locale->get_weekday($service->service_day),esc_html($service->service_time));
