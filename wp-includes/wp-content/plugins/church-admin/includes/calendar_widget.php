...
{function church_admin_widget_control()
{
    ...
    if(!empty($_POST['widget_submit']))
    {
        ...
        if(!empty($_POST['description'])) {$options['description']='1';}else{$options['description']='0';}
...
function church_admin_widget_control_form()
{
    ...
    if(!empty($option['postit'])) echo ' checked="checked" ';
    echo '/></p>';
    echo '<p><label for="description">'.__('Show description','church-admin').'?:</label><input type="checkbox" name="description" value="1"';
    if(!empty($option['description'])) echo ' checked="checked" ';
    echo '/></p>';
...
if($postit)$out.='<div class="Postit">';
    ...
    //query
$sql='SELECT a.*,b.category FROM '.CA_DATE_TBL.' a, '.CA_CAT_TBL.' b WHERE '.$cat.' a.cat_id=b.cat_id AND a.start_date>="'.$sqlnow.'" AND a.general_calendar=1 ORDER by a.start_date,a.start_time LIMIT '.$limit;
//$out.= $sql;
  ...
  foreach($result AS $row)
    {
		...
		if($old_date!=$date){$out.='<h3>'.$date.'</h3>';}
		$out.='<p><strong>'.esc_html(mysql2date(get_option('time_format'),$row->start_time)).' - '.esc_html($row->title).'</strong></br/>';
		if(!empty($row->location))$out.=esc_html($row->location).'<br/>';
    if(!empty($options['description'])&&!empty($row->description))$out.=esc_html($row->description).'<br/>';
    	if(!empty($row->link))$out.='<div class="ca-day-link"><a href="'.esc_url($row->link).'">'.esc_html($row->link_title).'</a></div>';
    $out.='</p>';
		$old_date=$date;
    }
