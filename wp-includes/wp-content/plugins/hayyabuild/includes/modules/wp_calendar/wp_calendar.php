 ...
 * @since      1.0.0
 * @package    hayyabuild
 * @subpackage hayyabuild/includes/modules/wp_calendar
 * @author     ZintaThemes <>
 */
class HayyaModule_wp_calendar
{
	/**
	...
	public function __construct() {
		if( class_exists('WP_Widget_Calendar') ) {
			...
			$this->name 		= __('Wordpress Calendar', HAYYAB_BASENAME);
			$this->description 	= __('Wordpress Calendar Widget', HAYYAB_BASENAME);
			$this->activated 	= true;

			if ( ! HayyaHelper::__current_user('manage_options') ) add_shortcode( 'hb_Widget_Calendar', array( $this, 'public_render' ) );

			$this->params = array(
// 					'color' => array(
// 							'type' => 'dropdown',
// 							'heading' => __('Calendar theme', HAYYAB_BASENAME),
// 							'description' => __('Select Calendar theme.', HAYYAB_BASENAME),
// 							'value' => array(
...
// 					),
					...
					'hhh' => array('type' => 'hiddenfield', 'value' => 'hhhh'),
			);
		}
	}
...
// 				)
		...
		$calendar = $param['title'].$param['before_title'].$param['after_title'].$param['before_widget'].$param['after_widget'];
		$html = '<div id="'.$param['id'].'" class="hb_widget '.$param['class'].'" style="'.$param['style'].'">
					<div class="hayya_hide_from_backend">
						[hb_Widget_Calendar '.$calendar.']
					</div>
