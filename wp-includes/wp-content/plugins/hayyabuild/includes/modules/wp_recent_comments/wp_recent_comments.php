 ...
 * @since      1.0.0
 * @package    hayyabuild
 * @subpackage hayyabuild/includes/modules/wp_recent_comments
 * @author     ZintaThemes <>
 */
class HayyaModule_wp_recent_comments
{
	/**
	...
	public function __construct() {
		...
		if( class_exists('WP_Widget_Recent_Comments') ) {
			...
			$this->name 		= __('Recent Comments', HAYYAB_BASENAME);
			$this->description 	= __('Display to a list of recent comments', HAYYAB_BASENAME);
			$this->activated 	= true;

			if ( ! HayyaHelper::__current_user('manage_options') ) add_shortcode( 'hb_Widget_Recent_Comments', array( $this, 'public_render' ) );

			$this->params = array(
					'number' => array(
							'type' => 'integer_slider',
							'heading' => __('Number of comments', HAYYAB_BASENAME),
							'description' => __('Number of comments to show (at most 15). Default: 5.', HAYYAB_BASENAME),
							'min' => '1',
							'max' => '15',
							'step' => '1',
					),
...
// 			$this->css_files = array('WP_Widget_Recent_Comments' => 'css/style.css');
		}
	}
	...
	public function admin_render($param = null) {
		...
		$html = '<div id="'.$param['id'].'" class="hb_widget '.$param['class'].'" style="'.$param['style'].'">
					<div class="hayya_hide_from_backend">
						[hb_Widget_Recent_Comments '.$recent.']
					</div>
