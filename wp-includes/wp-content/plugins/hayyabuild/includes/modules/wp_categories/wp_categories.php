 ...
 * @since      1.0.0
 * @package    hayyabuild
 * @subpackage hayyabuild/includes/modules/wp_categories
 * @author     ZintaThemes <>
 */
class HayyaModule_wp_categories
{
	/**
	...
	public function __construct() {
		...
		if( class_exists('WP_Widget_Categories') ) {
			...
			$this->name 		= __('Categories widget', HAYYAB_BASENAME);
			$this->description 	= __('Displays a list of categories', HAYYAB_BASENAME);
			$this->activated 	= true;

			if ( ! HayyaHelper::__current_user('manage_options') ) add_shortcode( 'hb_Widget_Categories', array( $this, 'public_render' ) );
			...
			$this->params = array(
					...
					'hierarchical' => array(
							'type' => 'checkbox',
							'heading' => __('Subcategories Inside Parent', HAYYAB_BASENAME),
							'description' => __('Display sub-categories as nested items inside the parent category.', HAYYAB_BASENAME),
							'value' => array(
									'1' => __('Yes', HAYYAB_BASENAME)
							)
					),
					'dropdown' => array(
							'type' => 'checkbox',
							'heading' => __('Categories Dropdown', HAYYAB_BASENAME),
...
// 			$this->css_files = array('WP_Widget_Categories' => 'css/style.css');
		}
	}
	...
	public function admin_render($param = null) {
		$args = array(
				...
				'hierarchical' => ' hierarchical="'.$param['hierarchical'].'"',
		...
		$categorie = $param['title'].$param['count'].$param['title'].$param['dropdown'].$param['before_title'].$param['after_title'].$param['before_widget'].$param['after_widget'];

		$html = '<div id="'.$param['id'].'" class="hb_widget '.$param['class'].'" style="'.$param['style'].'">
					<div class="hayya_hide_from_backend">
						[hb_Widget_Categories '.$categorie.']
					</div>
        ...
        if ( $atts ) {
    		foreach ( $atts as $key => $value ) {
    			if ( !empty($value) ) {
    				...
    				else if ( $key ==  'hierarchical' ) $instance .= '&hierarchical='.$value;
