...
function arve_pro_sc_filter_img_src( $atts ) {
	...
	switch ( $atts['provider'] ) {
		case 'alugha':
			$atts['img_src'] = arve_pro_get_json_thumbnail( "https://api.alugha.com/v1/videos/$id", 'thumb', $atts );
			break;
		case 'dailymotionlist': // TODO Check if there are always 720p thumbnails.
			$atts['img_src'] = arve_pro_get_json_thumbnail( "https://api.dailymotion.com/playlist/$id?fields=thumbnail_720_url$id", 'thumbnail_720_url', $atts ); // TODO _url$id ??????
			break;
		...
		case 'twitch':
			if ( is_numeric( $id ) ) {
				$atts['img_src'] = arve_pro_get_json_thumbnail( 'https://api.twitch.tv/kraken/videos/v' . $id, 'preview', $atts );
			} else {
				$atts['img_src'] = arve_pro_get_json_thumbnail( 'https://api.twitch.tv/kraken/channels/' . $id, 'video_banner', $atts );
			}
			break;
		case 'liveleak':
...
function arve_pro_sc_filter_img_src_srcset( $atts ) {
	...
	if ( false === $srcset ) :
		...
		if ( 'youtube' === $atts['provider'] ) {
			...
			$fb_format_json = arve_pro_get_json_thumbnail( "https://graph.facebook.com/{$fb_vidid}/", 'format', $atts );
