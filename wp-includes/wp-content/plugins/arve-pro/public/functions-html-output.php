...
function arve_pro_filter_output( $output, $html, $atts ) {
	...
	if ( 'link-lightbox' === $atts['mode'] ) {
		...
		$html['embed_container'] = arve_pro_lity_container( $html['meta'] . $html['video'], $atts );
		...
		if ( 'html5' === $atts['provider'] ) {
			$html['embed_container'] .= arve_arve_embed_container( $html['meta'] . $html['video'] . arve_pro_play_btn( $atts ), $atts );
		} else {
			$html['embed_container'] .= arve_arve_embed_container( $html['meta'] . arve_pro_play_btn( $atts ), $atts );
		}
	} else {
